...
class RGBAColors(int, Enum):
    ...
    def __new__(cls, value, rgba='...'):
        """Create an object of RGBAColors given the parameter.
    ...
    @staticmethod
    def validate_rgba(rgba: list) -> list:
        """Validate the type and length of rgba format of a color and fix it if needed.

        :param rgba: the color in (r, g, b, a) format
        :type rgba: list
        :return: the corrected rgba
        :rtype: list
        """
...
def _create_marker(
    ...
    lifetime=None,
    pose: Union[list, Pose, PoseStamped, None] = None,
...
def _create_delete_marker(name: str, id: int, frame_id: str | None = None) -> Marker:
    """Return a Marker object specified to delete a marker.
...
def _create_delete_all_marker(frame_id: str | None = None) -> Marker:
    """Return a Marker object specified to delete all markers.
...
def _create_delete_all_marker_array(frame_id: str | None = None) -> MarkerArray:
    """Return a Marker object specified to delete all markers.
...
def create_axisplane_marker(
    ...
    :param lifetime: the duration that the marker is displayed, defaults to 0 (indefinte)
...
def create_cube_marker_from_bbox(name: str, id: int, bbox3d: list, frame_id: str | None = None, rgba: list | None = None, lifetime: float = 0) -> Marker:
    ...
    the_marker = _create_marker(name, id, Marker.CUBE, frame_id=frame_id, lifetime=lifetime, pose=pose, scale=scale, color=rgba)
    return the_marker


def create_cube_marker_from_xyzrpy(
    name: str, id: int, xyzrpy: list, frame_id: str | None = None, scale: float | list = 0.5, rgba: list | None = None, lifetime: float = 0
) -> Marker:
    ...
    :return: the Marker object
    """
    the_marker = _create_marker(name, id, Marker.CUBE, frame_id, lifetime, pose=xyzrpy, scale=scale, color=rgba)
    return the_marker


def create_arrow_marker_from_xyzrpy(
    name: str,
    id: int,
    xyzrpy: list,
    frame_id: str | None = None,
    arrow_length: float = 0.5,
    arrow_shaft_diameter: float = 0.1,
    arrow_head_diameter: float = 0.1,
    rgba: list | None = None,
    lifetime: float = 0,
) -> Marker:
...
def create_line_marker(
    name: str, id: int, xyz1: list, xyz2: list, frame_id: str | None = None, line_width: float = 0.01, rgba: list | None = None, lifetime: float = 0
) -> Marker:
...
def create_path_marker(
    name: str, id: int, xyzlist: list, frame_id: str | None = None, line_width: float = 0.01, rgba: list | None = None, lifetime: float = 0
) -> Marker:
...
def create_sphere_marker(
    name: str, id: int, xyzrpy: list, frame_id: str | None = None, scale: float | list = 0.2, rgba: list | None = None, lifetime: float = 0
) -> Marker:
...
def create_cylinder_marker(
    name: str, id: int, xyzrpy: list, frame_id: str | None = None, scale: float | list = (0.1, 0.1, 0.2), rgba: list | None = None, lifetime: float = 0
) -> Marker:
    ...
    :return: the Marker object
    """
    the_marker = _create_marker(name, id, Marker.CYLINDER, frame_id=frame_id, lifetime=lifetime, pose=xyzrpy, scale=scale, color=rgba)
    return the_marker


def create_text_marker(
    name: str, id: int, text: str, xyzrpy: list, frame_id: str | None = None, scale: list | float = 0.5, rgba: list | None = None, lifetime: float = 0
) -> Marker:
    ...
    :return: the Marker object
    """
    the_marker = _create_marker(name, id, Marker.TEXT_VIEW_FACING, frame_id=frame_id, lifetime=lifetime, pose=xyzrpy, scale=scale, color=rgba)
    the_marker.text = text
    return the_marker


def create_mesh_marker(
    name: str, id: int, resource_uri: str, xyzrpy: list, frame_id: str | None = None, scale: list | float = 0.5, rgba: list | None = None, lifetime: float = 0
) -> Marker:
    ...
    #     logger.warning("create_mesh_marker: scale should be a list of 3 numbers (radius, radius, height)")
    #     return None
    the_marker = _create_marker(name, id, Marker.MESH_RESOURCE, frame_id=frame_id, lifetime=lifetime, pose=xyzrpy, scale=scale, color=rgba)
    # try:
...
def create_pointcloud_from_image(
    ...
    xyz: list = (0, 0, 0),
    pixel_physical_size: float = 0.005,
    frame_id: str | None = None,
    opacity: float = 1.0,
    depth_array: Union[np.ndarray, None] = None,
) -> PointCloud2:
    ...
    if depth_array is not None and (image_bgr.shape[0] != depth_array.shape[0] or image_bgr.shape[1] != depth_array.shape[1]):
    ...
    cloud_data["y"] = (
        np.repeat(np.linspace(0, image_height, image_height) * pixel_physical_size[1], image_width) - image_height * pixel_physical_size[1]
    ) * -1 + xyz[1]
...
def _create_empty_pointcloud(frame_id: str | None = None) -> PointCloud2:
...
def update_marker_xyzrpy(marker: Marker, xyzrpy: list) -> None:
    ...
    :type xyzrpy: list
    """
    assert isinstance(xyzrpy, (list, tuple)) and len(xyzrpy) == 6, "invalid parameter (xyzrpy): expects a list of 6 numbers"
...
def move_marker(marker: Marker, xyz_offset: list) -> None:
    ...
    """
    assert isinstance(xyz_offset, (list, tuple)) and len(xyz_offset) == 3, "invalid parameter (xyz_offset): expects a list of 3 numbers"
...
class PublishTopicManager:
    ...
    def __init__(self, node: Node, default_qos_profile: QoSProfile):
        ...
        self.topics_of_messages = defaultdict(list)  # (message cls name) > list (topic name)

    @synchronized
    def add_topic_of_message_class(self, topic: str, message_cls: type, qos_profile: Union[QoSProfile, None] = None) -> Publisher:
        ...
        assert isinstance(message_cls, type), "TopicManager add_topic: invalid parameter type (message_cls) - requires a ros2 visualization message class"
        assert message_cls in (Marker, MarkerArray, PointCloud2), "TopicManager add_topic: invalid parameter value (message_cls) must be visualization.msgs"
        if topic in self.topics_dict:
            raise ValueError("invalid parameter (topic): the topic already used")
        # set qos_profile
    ...
    def delete_topic(self, topic: str) -> None:
        ...
        if topic not in self.topics_dict:
            raise ValueError("invalid parameter (topic): the topic not exists")
    ...
    def get_topics_list_of_messages(self, message_cls: type) -> list[str]:
        """Return the list of topics that are associated with the given message class and have been added to this manager.
    ...
    def get_message_cls_of_topic(self, topic: str) -> type:
        ...
        :rtype: type
        """
        message_cls, _ = self.topics_dict[topic]
        return message_cls
...
class RvizMarkerPublisher:
    """A publisher of visualization objects including Markers, MarkerArray and Pointcloud2.
    ...
    def __init__(self, node: Node, fixed_frame: str = "map", callback_group: CallbackGroup = None, default_qos_profile: QoSProfile = None, **config_dict):
        """Initialize the RvizMarkerPublisher.
        ...
        self.tf_queue_lock = threading.RLock()
        # input parameter
        self._node = node
        ...
        self._default_qos_profile = (
            QoSProfile(durability=QoSDurabilityPolicy.VOLATILE, reliability=QoSReliabilityPolicy.RELIABLE, history=QoSHistoryPolicy.KEEP_LAST, depth=50)
            if default_qos_profile is None
            else default_qos_profile
        )
        ...
        self.static_tf_broadcaster = tf2_ros.StaticTransformBroadcaster(
            self._node
        )  # default QosProfile is TRANSIENT_LOCAL, KEEP LAST and the queue length is 1
    ...
    def _get_object_model(self, the_object: Any) -> RvizObjectModel:
        """Lookup and return the internal RvizObjectModel given the object.
    ...
    def _get_ros_time_in_seconds(self, offset: float | None = None) -> float:
        """Return the ros time in seconds plus the offset if given.
        ...
        with self.object_queue_lock:
            ...
            for cached_object_model in list(self.cached_objects_queue):
                ...
                if the_publisher is not None:
                    ...
                    self.delete_object(the_object)

    # internal callback function to run the transform publishing task
    def _cb_timer_tf(self):
        with self.tf_queue_lock:
            custom_tf: RvizCustomTFModel
            for custom_tf in list(self.tfs_dict.values()):
                # the pose may be coming from a Marker object if this is linked to the Marker
                self._pub_transform(custom_tf.frame_id, custom_tf.pose, custom_tf.parent_frame_id, custom_tf.static_tf)
    ...
    def _pub_transform(self, frame_id: str, pose: Pose, parent_frame_id: str | None = None, static_tf: bool = False) -> TransformStamped:
        ...
        if type(pose) in [list, tuple]:
            ...
            logger.warning("(_pub_transform) parameter (pose) is not list of length 6 or 7 or a Pose object -> fix the parameter at behaviour construction")
            raise TypeError("A parameter is invalid")
    ...
    def _get_default_topic(self, the_object: Any) -> str:
        """Return the default topic according to the object type.
    ...
    def activate_topic(self, topic: str, message_cls: type, qos_profile: Union[QoSProfile, None] = None) -> None:
    ...
    def publish_and_cache(
        self, the_object: Union[Marker, MarkerArray, PointCloud2], topic: str | None = None, pub_tf: bool = False, update_stamp: bool = True
    ) -> Union[Marker, MarkerArray, PointCloud2]:
        ...
        with self.object_queue_lock:
            if pub_tf and isinstance(the_object, Marker):
                ...
                self.cached_objects_queue.append(
                    RvizObjectModel(the_object=the_object, topic=topic, tf_frame_id=tf_frame_id, ns=the_object.ns, id=the_object.id, update_stamp=update_stamp)
                )
            elif isinstance(the_object, Marker):
                # if pub_tf is False and the object is a Marker
                self.cached_objects_queue.append(
                    RvizObjectModel(the_object=the_object, topic=topic, ns=the_object.ns, id=the_object.id, update_stamp=update_stamp)
                )
            else:
    ...
    def publish(
        self, the_object: Union[Marker, MarkerArray, PointCloud2], topic: str | None = None, delay: float = 0, update_stamp: bool = True
    ) -> Union[Marker, MarkerArray, PointCloud2]:
    ...
    def publish_custom_tf(self, frame_id: str, parent_frame_id: str, pose: Pose, static_tf: bool = False, linked_marker: Union[Marker, None] = None) -> None:
        ...
        if frame_id is None or parent_frame_id is None or pose is None:
            raise AssertionError("RvizVisualizer (add_custom_tf): No parameter can be None")
        ...
        if isinstance(linked_marker, Marker):
            with self.tf_queue_lock:
                self.tfs_dict[frame_id] = RvizCustomTFModel(
                    static_tf=static_tf,
                    frame_id=frame_id,
                    parent_frame_id=parent_frame_id,
                    pose=linked_marker.pose,
                    linked_marker=linked_marker,
                    pose_linked_marker=True,
                )
        else:
            with self.tf_queue_lock:
                self.tfs_dict[frame_id] = RvizCustomTFModel(
                    static_tf=static_tf, frame_id=frame_id, parent_frame_id=parent_frame_id, pose=pose, pose_linked_marker=False
                )

    def delete_marker_by_id(self, name: str, id: int) -> None:
        """Delete marker by its name and id.

        :param name: the namespace of the marker
        :type name: str
        :param id: the id of the marker
        :type id: int
        """
    ...
    def delete_object(self, the_object: Union[Marker, MarkerArray, PointCloud2]) -> None:
    ...
    def _delete_marker(self, the_object: Marker):
        ...
        with self.tf_queue_lock:
            if object_model is not None and object_model.tf_frame_id in self.tfs_dict:
    ...
    def delete_cached_objects_by_topics(self, topics_list: list | str | None = None) -> None:
    ...
    def delete_all_objects_by_topics(self, topics_list: list | str | None = None) -> None:
