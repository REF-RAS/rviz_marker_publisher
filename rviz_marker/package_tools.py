...
class PackageFile():
    ...
    def resolve_to_file_uri(uri_or_path:str) -> str:
        """
        Convert a uri or a file path into a uri starting with file://.
    ...
    def resolve_to_path(uri_or_path:str) -> str:
        """
        Convert a uri or a file path into a full local file path.
    ...
    def _search_rospkgs_for_file(path):
        packages_dict = get_packages_with_prefixes()
        for package_name in packages_dict:
            package_prefix = packages_dict[package_name]
            candidate_path = os.path.join(package_prefix, path)
...
if __name__ == '__main__':
    ...
    tests = ['/app/ros2_ws/src/task_trees/demos/rviz_display/utah_teapot.stl',
            'task_trees/demos/rviz_display/utah_teapot.stl',
            'file:///home/ubuntu/arm_commander_ws/src/task_trees/demos/rviz_display/utah_teapot.stl',
